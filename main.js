...
function resetGroupIdentity() {
  groupMembers = new Set()
  groupPets = new Map()
}

function groupIdentityEvent(ev) {
  if (!ev) return ev
  if (ev.kind === 'group') {
    const name = ev.name?.trim()
    if (ev.change === 'selfJoin') groupMembers.add(combat?.playerName || 'You')
    if (ev.change === 'selfLeave') resetGroupIdentity()
    if (ev.change === 'join' && name) groupMembers.add(name)
    if (ev.change === 'leave' && name) {
      groupMembers.delete(name)
      for (const [pet, owner] of groupPets) if (owner.toLowerCase() === name.toLowerCase()) groupPets.delete(pet)
    }
  }
  if (ev.kind === 'allyPetLeader' && ev.pet && ev.owner) {
    groupPets.set(ev.pet.trim(), ev.owner.trim())
    groupMembers.add(ev.owner.trim())
  }
  if (ev.kind === 'petClaim' && ev.name) groupPets.set(ev.name.trim(), 'You')
  if (ev.kind === 'petSay' && ev.name && ev.owner) groupPets.set(ev.name.trim(), ev.owner.trim())
  return ev
}

function applyCombatIdentity(ev) {
  groupIdentityEvent(ev)
  if (ev.kind !== 'damage' && ev.kind !== 'heal') return ev

  const attacker = ev.attacker?.trim()
  const petOwner = attacker ? groupPets.get(attacker) : null
  const attackerIsGroupMember = attacker && [...groupMembers].some(n => n.toLowerCase() === attacker.toLowerCase())

  if (attackerIsGroupMember) {
    ev.entityType = 'you'
    ev.owner = attacker
  } else if (petOwner) {
    ev.entityType = petOwner === 'You' ? 'pet' : 'pet'
    ev.owner = petOwner
  }
  return ev
}
...
function subscribeModules(targetBus, targetCombat, targetTracker, targetAttribution, targetEncounters, targetFarming) {
  targetBus.subscribe((rawEv, live) => {
    let ev = targetAttribution.process(rawEv)
...
function buildSnapshot() {
  ...
  return {
    ...
    groupPets: [...groupPets.entries()].map(([pet, owner]) => ({ pet, owner }))
  }
}

function createWindow() {
  bus=new LogBus(); combat=new CombatModule(); tracker=new TrackerModule(); attribution=new AttributionModule(); encounters=new EncounterEngine(); farming=new FarmingModule(); resetGroupIdentity()
...
ipcMain.handle('parse-log-for-date',async(_,filePath)=>{
  const tempCombat=new CombatModule(),tempTracker=new TrackerModule(),tempAttribution=new AttributionModule(),tempEncounters=new EncounterEngine(),tempFarming=new FarmingModule(),tempBus=new LogBus();
  const oldCombat=combat, oldGroupMembers=groupMembers, oldGroupPets=groupPets
  combat=tempCombat; resetGroupIdentity(); subscribeModules(tempBus,tempCombat,tempTracker,tempAttribution,tempEncounters,tempFarming)
  try{const content=fs.readFileSync(filePath,'utf-8'),lines=content.split(/\r?\n/),nameMatch=path.basename(filePath).match(/^eqlog_([^_]+)_/i);if(nameMatch){tempCombat.setPlayerName(nameMatch[1]);tempTracker.player=nameMatch[1]}for(let i=0;i<lines.length;i++){if(!lines[i].trim())continue;const ev=parseRawLine(lines[i],i+1);if(ev)tempBus.emit(ev,true)}tempEncounters.onTick(Date.now());tempCombat.setEncounterContext(tempEncounters.current);const encounterSnapshot=tempEncounters.snapshot();const result={...tempCombat.snapshot(),tracker:tempTracker.snapshot(),encounters:encounterSnapshot.encounters||[],encounterActive:encounterSnapshot.active||null,farming:tempFarming.snapshot(),groupMembers:[...groupMembers],groupPets:[...groupPets.entries()].map(([pet,owner])=>({pet,owner}))};combat=oldCombat;groupMembers=oldGroupMembers;groupPets=oldGroupPets;return result}catch(err){combat=oldCombat;groupMembers=oldGroupMembers;groupPets=oldGroupPets;return{error:err.message}}
})
...
async function startParser(filePath) {
  if(tailInterval)clearInterval(tailInterval);combat.reset();tracker.reset();attribution.reset();encounters.reset();farming.reset();seq=0;parsing=true;resetGroupIdentity()
  try{const content=fs.readFileSync(filePath,'utf-8'),lines=content.split(/\r?\n/),nameMatch=path.basename(filePath).match(/^eqlog_([^_]+)_/i);if(nameMatch){combat.setPlayerName(nameMatch[1]);tracker.player=nameMatch[1];groupMembers.add(nameMatch[1])}for(const line of lines){if(!line.trim())continue;const ev=parseRawLine(line,++seq);if(ev)bus.emit(ev,true)}tailFile(filePath);saveLastLogPath(filePath);return{ok:true,path:filePath,lines:lines.length}}catch(err){console.error(err);return{ok:false,error:err.message}}finally{parsing=false}
}
...
ipcMain.handle('set-player-name',async(_,name)=>{combat.setPlayerName(name);if(name)groupMembers.add(name);tracker.player=name;return true})