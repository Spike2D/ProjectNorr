...
function fixedEvent(ctx) {
  ...
  if (m && !/\b(?:platinum|gold|silver|copper)\b/i.test(m[2])) return event(ctx, 'itemReceived', { item: m[2].trim(), count: m[1] ? Number(m[1]) : 1, via: 'receive' })
...
function parseEvent(raw, seq) {
  ...
  return fixedEvent(ctx) || base.parseEvent(raw, seq)
}
